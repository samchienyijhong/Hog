...
def free_bacon(score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    return 6 if score == 0 else pi // (10 ** (100 - score)) % 10 + 3
    # END PROBLEM 2
...
def pig_pass(player_score, opponent_score):
    ...
    if player_score < opponent_score:
        return True if opponent_score - player_score < 3 else False
    else:
        return False
    # END PROBLEM 4b
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    strategy = make_averaged(roll_dice, trials_count)
    result = 0
    for i in range(1, 11):
        temp = strategy(i, dice)
        if temp > result:
            result = temp
            num = i
    return num
